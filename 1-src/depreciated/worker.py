...
def main_worker(
    ...
    ckpt_path: str = config["training"].get("ckpt_path", None)
    if ckpt_path is not None:
        ...
        if model_state_dict:
            ...
            worker_logger.debug(f"Model state updated from checkpoint at {ckpt_path}.")
    ...
    if "scheduler" in config:
        ...
        amp_enabled: bool = config["amp"]["amp_enabled"]
    ...
    verbose_print_interval: int = config["training"].get("verbose_print_interval", 100)
    ...
    worker_logger.debug(
        ...
        f'Printing frequency set to every {verbose_print_interval} iterations, '
    ...
    trainer = Trainer(
        ...
        verbose=verbose,
        val_enabled=config["evaluation"]["val_enabled"],
    ...
    top1 = None
    if gpu == 0 and config["evaluation"]["eval_enabled"]:
