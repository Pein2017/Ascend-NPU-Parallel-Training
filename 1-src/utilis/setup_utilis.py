...
from global_settings import DEFAULT_LOGGER_DIR  # noqa


def setup_logger(
    name: str,
    log_file_name: str,
    logger_dir: str = DEFAULT_LOGGER_DIR,
    ...
    if logger_dir is None:
        raise ValueError(
            "Logger directory is not set. Please set the logger directory."
        )
    ...
    if console:
        ...
        # print(f"Added stream handler to logger: {name}")

    if not os.path.exists(logger_dir):
        os.makedirs(logger_dir, exist_ok=True)
        print(f"Created log directory: {logger_dir}")

    full_path = os.path.join(logger_dir, log_file_name)
    # print(f"Full path for log file: {full_path}")

    file_handler = logging.FileHandler(full_path, mode="a")
    file_handler.setFormatter(formatter)
    logger.addHandler(file_handler)
    # print(f"Added file handler for log file: {full_path}")
