...
class Worker:
    ...
    def extract_parameters(self):
        ...
        self.verbose_print_interval = training_config["verbose_print_interval"]
        ...
        self.track_model_stats = training_config["track_model_stats"]
        self.ckpt_dir = training_config["ckpt_dir"]
        self.ckpt_path = training_config["ckpt_path"]
        self.ckpt_save_interval = training_config["ckpt_save_interval"]

        # debug
        if self.ckpt_save_interval < 100:
            raise ValueError("ckpt_save_interval should be larger than 100")
        ...
        self.eval_enabled = evaluation_config["eval_enabled"]
        self.val_enabled = evaluation_config["val_enabled"]

        # AMP
        amp_config = self.config["amp"]
        self.amp_enabled = amp_config["amp_enabled"]
    ...
    def load_checkpoint(self):
        ...
        if ckpt_path:
            ...
            state = load_checkpoint(ckpt_path=ckpt_path)
            if state:
                ...
                if "model_state_dict" in state:
                    ...
                    self.worker_logger.debug(
                        f"Model state updated from checkpoint at {ckpt_path}."
                    )
    ...
    def setup_benchmark_and_stats_tracker(self):
        ...
        self.worker_logger.debug(
            ...
            f'Printing frequency set to every {self.verbose_print_interval} iterations, '
        ...
        if self.eval_enabled:
            ...
            model_stats_tracker, losses_meter, top1, top5 = (
                self.trainer.evaluate_one_epoch(
                    -1, self.test_loader, "Final test", True
                )
            )

        if self.rank == 0 and self.eval_enabled:
